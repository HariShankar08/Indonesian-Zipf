...
if not os.path.exists('tokens_randomized.txt'):
    os.system("tr -sc 'A-Za-z' '\n' < randomized.txt | tr '[:upper:]' '[:lower:]' | sort | uniq -c | sort -nr > tokens_randomized.txt")

if not os.path.exists('characters_randomized.txt'):
    os.system("tr '[:upper:]' '[:lower:]' < randomized.txt | tr -sc 'A-Za-z' '\n' | egrep -o '.' | sort | uniq -c | sort -nr > characters_randomized.txt")

with open('tokens_randomized.txt', 'r') as f:
...
plt.plot(freq)
plt.savefig('randomized/words_freq.png')
plt.show()

with open('randomized/correlation_words.txt', 'w') as f:
...
plt.savefig('randomized/words_logfreq.png')
plt.show()

with open('characters_randomized.txt', 'r') as f:
    # rank = index of word in list + 1
    # (word, freq) == items of list
...
plt.plot(freq)
plt.savefig('randomized/char_freq.png')
plt.show()
...
plt.savefig('randomized/char_logfreq.png')
plt.show()

with open('randomized/correlation_characters.txt', 'w') as f:
